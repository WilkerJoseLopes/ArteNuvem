...
def index():
    ...
    return render_template("index.html", fotos=fotos, imagens=imagens, categorias=categorias, other_cats=other_cats, query_text=q, selected_categoria=categoria_id)
...
def publicar():
    if request.method == "POST":
        ...
        img = Imagem(titulo=titulo, caminho_armazenamento=caminho_url, categoria_texto=categoria_obj.nome if categoria_obj else None, id_utilizador=autor.id, id_categoria=categoria_id if categoria_id else None)
        if tags:
            img.tags = tags
        db.session.add(img)
        db.session.commit()
...
@app.route("/exposicao")
def exposicao():
    top = db.session.query(Imagem, db.func.count(Voto.id).label("total_votos")).outerjoin(Voto, Voto.id_imagem == Imagem.id).group_by(Imagem.id).order_by(db.desc("total_votos")).limit(10).all()
